...
Все операции по развёртке инфраструктуры выполнялись в Terraform и Ansible.
Ansible был установлен на Bastion host, ssh подключение к ВМ велось также с Bastion host.
...
Согласно требованиям, web сервера и elasticsearch во время развёртки были помещены во внутреннюю подсеть:





Сервера Zabbix, Kibana, балансировщика и Bastion host во время развёртки были помещены в публичную подсеть:



Пробовал настроить HTTPS в инфраструктуре, но так окончательно и не разобрался с настройкой Бакетов и Балансировщика через Terraform. Я так и не нашёл правильной версии написания файла терраформа для балансировщика с перенаправлением на HTTPS
